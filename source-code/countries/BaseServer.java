...
public class BaseServer
{
    ...
    static {
        try {
            Document doc = DocumentBuilderFactory.newInstance().newDocumentBuilder()
                .parse(new File("source-code/countries/config.xml"));
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("[countries] port range: " + portStart + "-" + portEnd);
        for (String[] inst : activeInstances) {
            String name = inst[0];
            int port = Integer.parseInt(inst[1]);
            new Thread(() -> {
                try (ServerSocket ss = new ServerSocket(port)) {
                    System.out.println("[countries." + name + "] listening on port " + port);
                    while (true) {
    ...
    private static void handle(String name, Socket client) {
        try (DataInputStream in   = new DataInputStream(client.getInputStream());
            ...
            out.writeUTF("countries." + name + ": received " + len + " samples");
        } catch (Exception e) { e.printStackTrace(); }
